...
def getProduct(request):
	if request.is_ajax():			
		...
		sendData = product.values('prd_cod','prd_nom','prd_pre','prd_des','prd_nro_piezas','prd_est','cat_cod__cat_nom' , 'cat_cod' , 'prd_url')
...
def getCategory(request):
	if request.is_ajax():
		category = Categoria.objects.all().order_by('cat_cod').filter(cat_est = True)
...
def saveCategory(request):
	if request.is_ajax():
		if request.POST['edit'] == '1': #MODIFICAR
			...
			if request.POST['withImage'] ==	"true":
				category.cat_url 	=	request.FILES['cat_url']
			category.save()
		...
		#=======GUARDAR ===========
			...
			if request.POST['withImage'] ==	"true":
				...
				category.save()
			return HttpResponse(json.dumps({"mensaje":"Guardada con éxito"}),content_type= "application/json : charset=utf-8")
	else:
		raise Http404
